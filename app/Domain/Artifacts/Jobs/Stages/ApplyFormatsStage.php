...
class ApplyFormatsStage implements StageInterface
{
    ...
    public function execute(ArtifactCommitAttempt $attempt): StageResult
    {
        ...
        foreach ($sheets as $sheet) {
            ...
            // Fetch formats
            $formatsIterable = $this->reader->iterateFormats($draft, $sheet->uuid);
            $batches = $this->planner->planFormatting($draft, $sheet->uuid, $formatsIterable, $capabilities);

            foreach ($batches as $batchIndex => $batch) {
                $checkpointKey = "formats_{$sheet->uuid}_{$batchIndex}";
